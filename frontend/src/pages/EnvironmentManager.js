...
import { environmentGroupsService, releaseService } from '../services/api';
import './EnvironmentManager.css';

const EnvironmentManager = ({ embedded = false, onNavigateToDetail }) => {
  ...
  const [sortBy, setSortBy] = useState(() => {
    return localStorage.getItem('EnvironmentManager_sortBy') || 'name';
  });
  const [sortOrder, setSortOrder] = useState(() => {
    return localStorage.getItem('EnvironmentManager_sortOrder') || 'asc';
  });
  // Load filter from localStorage or default to 'all'
  const [releaseTypeFilter, setReleaseTypeFilter] = useState(() => {
    return localStorage.getItem('EnvironmentManager_typeFilter') || 'all';
  });
  ...
  useEffect(() => {
    if (environmentGroupsService.length > 0) {
      const defaultExpanded = {};
      environmentGroupsService.forEach(group => {
  ...
  const loadEnvironmentGroups = async () => {
    try {
      setLoading(true);
      const data = await environmentGroupsService.getEnvironmentGroups();
      setEnvironmentGroups(data);
      setError(null);
    } catch (err) {
      setError('Failed to load environment groups: ' + err.message);
    } finally {
      setLoading(false);
    }
  }

  const loadEnvironments = async (groupId) => {
    try {
      setLoading(true);
      const data = await environmentGroupsService.getSpecificEnvironmentGroup(groupId);
  ...
  const handleReleaseTypeFilterChange = (newFilter) => {
    ...
    localStorage.setItem('EnvironmentManager_typeFilter', newFilter);
  };
        ...
        <div className="sort-controls">
          <select
            ...
            onChange={(e) => {
              ...
              localStorage.setItem('EnvironmentManager_sortBy', e.target.value);
            }}
          ...
          <button
            onClick={() => {
              ...
              localStorage.setItem('EnvironmentManager_sortOrder', newOrder);
            }}
